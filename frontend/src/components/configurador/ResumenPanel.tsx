...
import { formatearPrecio, formatearConsumo } from '../../utils/formatters';
import { PASOS_CONFIGURADOR, Configuracion } from '../../types';
...
const ResumenPanel: React.FC = () => {
    ...
    const { componentesSeleccionados, precioTotal, consumoEstimado, guardando, compatibilidad } =
    ...
    // Evitamos mostrar "✅ Configuración compatible" hasta que el backend
    // haya realmente validado — `compatibilidad` solo existe tras la llamada.
    const mostrarExito =
        componentesCount >= 2 &&
        !tieneErrores &&
        !validando &&
    ...
    const handleGuardar = async () => {
        ...
        if (guardarConfiguracion.fulfilled.match(result)) {
            toast.mostrar('Configuración guardada exitosamente', 'success');
        } else {
            toast.mostrar('No se pudo guardar la configuración', 'error');
        }
    };
