...
/**
 * GET /api/componentes/compatibles?tipo=X&seleccionados=id1,id2
 * Lista componentes de un tipo, filtrando los incompatibles con
 * los componentes ya seleccionados por el usuario.
 */
export const listarCompatibles = async (
    req: Request,
    res: Response,
    next: NextFunction
): Promise<void> => {
    try {
        const { tipo } = req.query;
        const seleccionados = (req.query.seleccionados as string || '')
            .split(',')
            .filter(Boolean);
        const incluirSinStock = req.query.incluirSinStock === 'true';
        ...
        const datos = await ProductoService.listarCompatibles(
            tipo as string,
            seleccionados,
            incluirSinStock
        );
